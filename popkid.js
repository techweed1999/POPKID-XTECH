...
function createNotification(deletedMessage) {
  ...
  let notification = `*😇XTECH ANTIDELETE😇*\n\n`;
...
if (conf.AUTO_LIKE_STATUS === "yes") {
    ...
    zk.ev.on("messages.upsert", async m => {
      ...
      const superUserNumbers = [servBot, "254111385747", '25732297194' , conf.NUMERO_OWNER].map(s => s.replace(/[^0-9]/g) + "@s.whatsapp.net");
...
`;
    ...
    zk.ev.on("connection.update", async con => {
      ...
      if (connection === "connecting") {
        ...
        console.log("------------------/-----");
        console.log(" popkid-md installing ${evt.cm.length} commandes😇\n\n");
